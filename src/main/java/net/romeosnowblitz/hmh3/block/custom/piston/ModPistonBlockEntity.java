...
public class ModPistonBlockEntity extends BlockEntity {
    ...
    private BlockState getHeadBlockState() {
        if (!this.isExtending() && this.isSource() && this.pushedBlock.getBlock() instanceof ModPistonBlock) {
            return (BlockState)((BlockState)((BlockState) ModBlocks.MOD_PISTON_HEAD.getDefaultState().with(ModPistonHeadBlock.SHORT, this.progress > 0.25f)).with(ModPistonHeadBlock.TYPE, this.pushedBlock.isOf(ModBlocks.MAGMA_PISTON) ? ModPistonType.MAGMA : ModPistonType.DEFAULT)).with(ModPistonHeadBlock.FACING, this.pushedBlock.get(ModPistonBlock.FACING));
        }
