...
public class ModPistonBlock extends FacingBlock {
    ...
    public ModPistonBlock(boolean magma, AbstractBlock.Settings settings) {
        super(settings);
    ...
    private void tryMove(World world, BlockPos pos, BlockState state) {
        ...
        if (bl && !state.get(EXTENDED).booleanValue()) {
            ...
            ModPistonBlockEntity modPistonBlockEntity;
            BlockEntity blockEntity;
            BlockPos blockPos = pos.offset(direction, 2);
            BlockState blockState = world.getBlockState(blockPos);
            int i = 1;
            if (blockState.isOf(ModBlocks.MOD_MOVING_PISTON) && blockState.get(FACING) == direction && (blockEntity = world.getBlockEntity(blockPos)) instanceof ModPistonBlockEntity && (modPistonBlockEntity = (ModPistonBlockEntity)blockEntity).isExtending() && (modPistonBlockEntity.getProgress(0.0f) < 0.5f || world.getTime() == modPistonBlockEntity.getSavedWorldTime() || ((ServerWorld)world).isInBlockTick())) {
                i = 2;
            }
    ...
    public boolean onSyncedBlockEvent(BlockState state, World world, BlockPos pos, int type, int data) {
        ...
        if (type == 0) {
            ...
            BlockState blockState = (BlockState)((BlockState)ModBlocks.MOD_MOVING_PISTON.getDefaultState().with(ModPistonExtensionBlock.FACING, direction)).with(ModPistonExtensionBlock.TYPE, this.magma ? ModPistonType.MAGMA : ModPistonType.DEFAULT);
            ...
            if (this.magma) {
                ...
                boolean bl2 = false;
                if (blockState2.isOf(ModBlocks.MOD_MOVING_PISTON) && (blockEntity2 = world.getBlockEntity(blockPos)) instanceof ModPistonBlockEntity && (modPistonBlockEntity = (ModPistonBlockEntity)blockEntity2).getFacing() == direction && modPistonBlockEntity.isExtending()) {
                    modPistonBlockEntity.finish();
                    bl2 = true;
                }
                if (!bl2) {
                    if (type == 1 && !blockState2.isAir() && ModPistonBlock.isMovable(blockState2, world, blockPos, direction.getOpposite(), false, direction) && (blockState2.getPistonBehavior() == PistonBehavior.NORMAL || blockState2.isOf(Blocks.PISTON) || blockState2.isOf(Blocks.STICKY_PISTON) || blockState2.isOf(ModBlocks.MAGMA_PISTON))) {
    ...
    public static boolean isMovable(BlockState state, World world, BlockPos pos, Direction direction, boolean canBreak, Direction pistonDir) {
        ...
        if (state.isOf(Blocks.PISTON) || state.isOf(Blocks.STICKY_PISTON) || state.isOf(ModBlocks.MAGMA_PISTON)) {
            if (state.get(EXTENDED).booleanValue()) {
                return false;
            }
        } else {
    ...
    private boolean move(World world, BlockPos pos, Direction dir, boolean retract) {
        ...
        BlockPos blockPos3;
        int k;
        ModPistonHandler modPistonHandler;
        ...
        if (!(modPistonHandler = new ModPistonHandler(world, pos, dir, retract)).calculatePush()) {
            return false;
        }
        HashMap<BlockPos, BlockState> map = Maps.newHashMap();
        List<BlockPos> list = modPistonHandler.getMovedBlocks();
        ...
        List<BlockPos> list3 = modPistonHandler.getBrokenBlocks();
        ...
        if (retract) {
            ModPistonType modPistonType = this.magma ? ModPistonType.MAGMA : ModPistonType.DEFAULT;
            BlockState blockState4 = (BlockState)((BlockState)ModBlocks.MOD_PISTON_HEAD.getDefaultState().with(ModPistonHeadBlock.FACING, dir)).with(ModPistonHeadBlock.TYPE, modPistonType);
            BlockState blockState = (BlockState)((BlockState)ModBlocks.MOD_MOVING_PISTON.getDefaultState().with(ModPistonExtensionBlock.FACING, dir)).with(ModPistonExtensionBlock.TYPE, this.magma ? ModPistonType.MAGMA : ModPistonType.DEFAULT);
            map.remove(blockPos);
